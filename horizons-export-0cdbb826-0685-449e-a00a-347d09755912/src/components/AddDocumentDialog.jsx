...
const AddDocumentDialog = ({ open, onOpenChange, categories, onAdd, userRoles, currentUserRole }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      onAdd(formData);
      setFormData({ name: '', category: '', status: 'Available', assignedTo: currentUserRole || '' });
      onOpenChange(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg bg-white dark:bg-slate-800 rounded-lg shadow-xl">
        <DialogHeader className="pb-4 border-b border-gray-200 dark:border-slate-700">
          <DialogTitle className="text-xl font-semibold text-gray-800 dark:text-slate-100">Add New Document</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-6">
          ...
          <DialogFooter className="mt-8 pt-6 border-t border-gray-200 dark:border-slate-700">
            ...
            <Button type="submit" className="ml-2 bg-gradient-to-r from-blue-600 to-sky-500 hover:from-blue-700 hover:to-sky-600 text-white shadow-md">Add Document</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
